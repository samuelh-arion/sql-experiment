...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'processing_expanded_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
...
def process_questions_parallel():
    """Process all questions from the expanded dataset in parallel."""
    # Read the expanded dataset
    df = pd.read_csv("dataset-expanded.csv")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        future_to_question = {
            executor.submit(
                process_single_question, row["alternative-question"], row["sql"]
            ): row["alternative-question"]
    ...
    output_file = f"results_expanded.json"
...
if __name__ == "__main__":
    logger.info("Starting parallel question processing for expanded dataset")
    results = process_questions_parallel()
    logger.info("Completed parallel question processing for expanded dataset")
